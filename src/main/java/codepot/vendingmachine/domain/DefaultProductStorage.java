...
public class DefaultProductStorage implements ProductStorage {
    ...
    @Inject
    public DefaultProductStorage(List<ServiceNotifier> serviceNotifiers) {
        this(serviceNotifiers,
                Lists.newArrayList(DefaultProducts.CANDY, DefaultProducts.CHIPS, DefaultProducts.COLA));
    }

    public DefaultProductStorage(List<ServiceNotifier> serviceNotifiers, List<Product> products) {
    ...
    public Product getProduct(String productCode) {
        ...
        if (productsQuantity.get(productCode) < 1) {
            serviceNotifiers.forEach(s -> s.doNotify());
        }
