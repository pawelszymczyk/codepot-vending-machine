...
public class ProductStorage {
    ...
    @Inject
    public ProductStorage(List<ServiceNotifier> notifiers) {
        this(notifiers,
                Lists.newArrayList(DefaultProducts.CANDY, DefaultProducts.CHIPS, DefaultProducts.COLA));
    }

    public ProductStorage(List<ServiceNotifier> notifiers, List<Product> products) {
    ...
    public Product getProduct(String productCode) {
        ...
        if(productsQuantity.get(productCode) < 1){
            this.notifiers.forEach(ServiceNotifier::doNotify);
        }
        return product;
    }
